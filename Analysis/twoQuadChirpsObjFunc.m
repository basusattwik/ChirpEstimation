...
s_measured_no_damping = SNR_increase_factor * (...
    A1_true * exp(-b * t) .* exp(1i * 2 * pi * (f01_true * t + k1_true * t.^2 + c1_true * t.^3)) + ...
    A2_true * exp(-b * t) .* exp(1i * 2 * pi * (f02_true * t + k2_true * t.^2 + c2_true * t.^3)));
...
for i = 1:length(f01_range)
    for j = 1:length(f02_range)
        s_model = SNR_increase_factor * (...
            A1_true * exp(1i * 2 * pi * (f01_range(i) * t + k1_true * t.^2 + c1_true * t.^3)) + ...
            A2_true * exp(1i * 2 * pi * (f02_range(j) * t + k2_true * t.^2 + c2_true * t.^3)));
        mse_f(i, j) = mean(abs(s_measured_no_damping - s_model).^2);
...
for i = 1:length(k1_range)
    for j = 1:length(k2_range)
        s_model = SNR_increase_factor * (...
            A1_true * exp(1i * 2 * pi * (f01_true * t + k1_range(i) * t.^2 + c1_true * t.^3)) + ...
            A2_true * exp(1i * 2 * pi * (f02_true * t + k2_range(j) * t.^2 + c2_true * t.^3)));
        mse_k(i, j) = mean(abs(s_measured_no_damping - s_model).^2);

        for n = 1:Ngs
            s_model_gs = SNR_increase_factor * (...
                A1_true * exp(1i * 2 * pi * (f01_true * t + (k1_range(i) + sig_gs*randn(1,1)) * t.^2 + c1_true * t.^3)) + ...
                A2_true * exp(1i * 2 * pi * (f02_true * t + (k2_range(j) + sig_gs*randn(1,1)) * t.^2 + c2_true * t.^3)));
            temp = temp + mean(abs(s_measured_no_damping - s_model_gs).^2);
        end
...
for i = 1:length(c1_range)
    for j = 1:length(c2_range)
        s_model = SNR_increase_factor * (...
            A1_true * exp(1i * 2 * pi * (f01_true * t + k1_true * t.^2 + c1_range(i) * t.^3)) + ...
            A2_true * exp(1i * 2 * pi * (f02_true * t + k2_true * t.^2 + c2_range(j) * t.^3)));
        mse_c(i, j) = mean(abs(s_measured_no_damping - s_model).^2);

        for n = 1:Ngs
            s_model_gs = SNR_increase_factor * (...
                A1_true * exp(1i * 2 * pi * (f01_true * t + k1_true * t.^2 + (c1_range(i) + sig_gs*randn(1,1)) * t.^3)) + ...
                A2_true * exp(1i * 2 * pi * (f02_true * t + k2_true * t.^2 + (c2_range(j) + sig_gs*randn(1,1)) * t.^3)));
            temp = temp + mean(abs(s_measured_no_damping - s_model_gs).^2);
        end
...
nexttile
surf(f01_range, f02_range, mse_f_gs', 'EdgeColor', 'none');
hold on;
contour(f01_range, f02_range, mse_f_gs', 10);
scatter3(f01_true, f02_true, min(mse_f_gs(:)), 100, 'r', 'filled');
xlabel('Frequency f_{01}');
ylabel('Frequency f_{02}');
zlabel('J');
title('3D MSE Smoothed for Varying f_{01} and f_{02} (No Damping)');

% 3D Plot MSE for k1 and k2
nexttile
surf(k1_range, k2_range, mse_k_gs', 'EdgeColor', 'none');
hold on;
contour(k1_range, k2_range, mse_k_gs', 10);
scatter3(k1_true, k2_true, min(mse_k_gs(:)), 100, 'r', 'filled');
xlabel('Chirp Rate k_1');
ylabel('Chirp Rate k_2');
zlabel('J');
title('3D MSE for Varying k_1 and k_2 (No Damping)');
...
% 3D Plot MSE for c1 and c2
nexttile
surf(c1_range, c2_range, mse_c_gs', 'EdgeColor', 'none');
hold on;
contour(c1_range, c2_range, mse_c_gs', 10);
scatter3(c1_true, c2_true, min(mse_c_gs(:)), 100, 'r', 'filled');
xlabel('Cubic Phase Coefficient c_1');
ylabel('Cubic Phase Coefficient c_2');
zlabel('J');
title('3D MSE for Varying c_1 and c_2 (No Damping)');
