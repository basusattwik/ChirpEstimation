classdef classGenPlots_SL < handle
    ...
    methods
        ...
        function genAllPlots(obj)
            ...
            nexttile
            for pind = 1:obj.numParticles
                plot(nx, obj.hessTrace(pind,:), 'LineWidth', 1.1, 'DisplayName', ['particle ', num2str(pind)]); hold on;
                grid on; grid minor;
                xlabel('Iterations', 'FontSize', 12); 
                ylabel('Hessian Trace', 'FontSize', 20, 'Interpreter','latex'); 
            ...
            nexttile
            for pind = 1:obj.numParticles
                plot(nx, obj.noiseVar(pind,:), 'LineWidth', 1.1, 'DisplayName', ['particle ', num2str(pind)]); hold on;
                grid on; grid minor;
                xlabel('Iterations', 'FontSize', 12); 
