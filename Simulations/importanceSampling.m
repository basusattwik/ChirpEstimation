...
mu = 0.8; 
sigma = sqrt(1.5); %est variance is sensitive to q(x)~N(mu,sigma)
...
figure; 
xx=linspace(0,8,100);
plot(xx, p(xx),'-r','linewidth',1.0); hold on; plot(xx, q(xx),'-b','linewidth',1.0); hold on;
plot(xx, p(xx)./q(xx),'--k','linewidth',1.0); hold on; plot(xx, p(xx).*f(xx),'-g','linewidth',1.0); hold on; grid on;
